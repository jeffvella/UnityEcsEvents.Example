...
namespace Assets.Scripts.Systems
{
    ...
    public class SimpleCollisionSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            Entities.ForEach((Entity entity, ref CollisionDetector detector, in Translation pos, in OwnerRef owner) => { detectors.Add(new DetectorInfo {Entity = entity, Position = pos.Value, Range = detector.Range, Owner = owner}); }).Run();
            ...
            Entities.WithNone<CollisionDetector>().ForEach((Entity entity, ref ActorDefinition def, ref Translation pos) =>
            {
                for (var i = 0; i < detectors.Length; i++)
                {
                    ...
                    {
                        var hit = new Hit {Position = detector.Position + (pos.Value - detector.Position), Target = entity};
                        var collision = new CollisionEvent {Source = detector, Hit = hit};
