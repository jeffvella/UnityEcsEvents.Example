...
public static class NavMeshCustomExtensions
{
    ...
    public static EdgeHit FindClosestEdge(NativeArray<Edge> edges, Vector3 position)
    {
        ...
        var result = new EdgeHit {Distance = closestDist, Position = closestPoint, Edge = closestEdge};
    ...
    public static void CalculateEdges(NavMeshTriangulation tr)
    {
        ...
        for (var i = 0; i < tr.indices.Length - 1; i += 3)
        {
            var v1 = tr.vertices[tr.indices[i]];
            var v2 = tr.vertices[tr.indices[i + 1]];
            var v3 = tr.vertices[tr.indices[i + 2]];
    ...
    public static bool Approx(Vector3 lhs, Vector3 rhs)
    {
        ...
        return num1 * (double) num1 + num2 * (double) num2 + num3 * (double) num3 < 0.01f;
    }
    ...
    public struct Edge
    {
        ...
        {
            return new Edge {Start = start, End = end, Center = (end + start) / 2, Direction = (end - start).normalized};
        }
