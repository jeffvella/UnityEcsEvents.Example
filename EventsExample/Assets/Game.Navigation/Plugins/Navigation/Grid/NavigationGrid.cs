...
namespace Providers.Grid
{
    public sealed class NavigationGrid : GridBase, IDisposable
    {
        ...
        private int GRID_BOUNDS { get; } = 50;
        ...
        // public ref GridNode this[GridNode node] => ref GetNearestNodeRef(ToGridDistance(node.X), ToGridDistance(node.Y), ToGridDistance(node.Z));
        //
        // public ref GridNode this[int3 node] => ref GetNearestNodeRef(ToGridDistance(node.x), ToGridDistance(node.y), ToGridDistance(node.z));
        //
        // public ref GridNode this[float3 node] => ref GetNearestNodeRef(ToGridDistance(node.x), ToGridDistance(node.y), ToGridDistance(node.z));
        ...
        public static NavigationGrid Create(int sizeX, int sizeY, int sizeZ, Vector3 position, Quaternion rotation, Vector3 scale, float boxSize = 1f)
        {
            ...
            for (var x = min.x + len; x < Max.x; x += boxSize)
            for (var y = min.y + len; y < Max.y; y += boxSize)
            for (var z = min.z + len; z < Max.z; z += boxSize)
            {
            ...
            //grid.Data = new GridViewData {BoxSize = boxSize, Min = new int3(grid.MinX, grid.MinY, grid.MinZ), Max = new int3(grid.MaxX, grid.MaxY, grid.MaxZ)};
        ...
        /*public static NavigationGrid CreateEmpty(int sizeX, int sizeY, int sizeZ, Vector3 position, Quaternion rotation, Vector3 scale, float boxSize = 1f)
        {
            ...
            grid.GridMaxX = grid.InnerGrid.Size.x - 1;
            grid.GridMaxY = grid.InnerGrid.Size.y - 1;
            grid.GridMaxZ = grid.InnerGrid.Size.z - 1;
        ...
        public IEnumerable<GridPoint> GetRayLine(Vector3 from, Vector3 to, float limitDistance = 0)
        {
            ...
            return Bresenham.GetPointsOnLine(gridFrom.X, gridFrom.Y, gridFrom.Z, gridTo.X, gridTo.Y, gridTo.Z).Select(p => new GridPoint((int) p.x, (int) p.y, (int) p.z));
        }
        ...
        {
            return Bresenham.GetPointsOnLine(start.X, start.Y, start.Z, end.X, end.Y, end.Z).Select(p => new GridPoint((int) p.x, (int) p.y, (int) p.z));
        }
        ...
        public GridNode FindClosestNode(Vector3 worldPosition, NodeFlags flags = default, int maxDistance = 10)
        {
            ...
            return flags == NodeFlags.None || nearestNodeInfo.Node.HasFlag(flags) ? nearestNodeInfo.Node : SearchNodes(nearestNodeInfo.GridPoint, maxDistance).FirstOrDefault(n => n.HasFlag(flags));
        }
