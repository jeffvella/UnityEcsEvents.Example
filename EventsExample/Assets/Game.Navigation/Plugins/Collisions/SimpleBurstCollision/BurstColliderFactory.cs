...
namespace Vella.SimpleBurstCollision
{
    public static class BurstColliderFactory
    {
        ...
        public static BurstSphereCollider CreateSphere(SphereCollider sphereCollider)
        {
            return new BurstSphereCollider {Scale = GetLargestAxis(sphereCollider.transform.localScale), Center = sphereCollider.transform.position, Radius = sphereCollider.radius, Offset = sphereCollider.center};
        }
        ...
        public static BurstSphereCollider CreateSphere(Vector3 center, float radius)
        {
            return new BurstSphereCollider {Center = center, Radius = radius, Scale = 1};
        }

        public static BurstSphereCollider CreateSphere(Vector3 center, Vector3 offset, float radius, float scale)
        {
            return new BurstSphereCollider {Center = center, Offset = offset, Radius = radius, Scale = scale};
        }
