...
namespace Vella.Common.Navigation
{
    ...
    public class BurstAStarPathFinder
    {
        ...
        public BurstAStarPathFinder GetPath(GridNode from, GridNode to)
        {
            ...
            var jobResult = PathFindingJob.Complete(this, ref Grid.InnerGrid, Areas, Id, from.GridPoint, to.GridPoint, AllowFlags, Grid.Transform.ToWorldMatrix);
        ...
        public struct PathFindingJob : IJob
        {
            ...
            public NodeFlags AllowedFlags;
            public int QueryId;
            public int MaxPoints;

            [NativeDisableUnsafePtrRestriction] public unsafe PathFindingJobResult* ResultPtr;

            public static unsafe (PathStatus PathStatus, List<GridNode> NodePath, List<Vector3> VectorPath) Complete(BurstAStarPathFinder pathFinder, ref NativeArray3D<GridNode> grid, INativeArrayProducer<NativeAreaDefinition> areas, int queryId, GridPoint start, GridPoint end, ulong allowedFlags, float4x4 localToWorld)
            {
                ...
                using (var nativePriorityQueue = new NativePriorityQueue(grid.Length / 8, Allocator.TempJob))
                {
                    ...
                    {
                        QueryId = queryId,
                        AllowedFlags = (NodeFlags) allowedFlags,
                        StartPoint = start,
                        EndPoint = end,
            ...
            public unsafe void Execute()
            {
                ...
                ref var start = ref Grid[StartPoint.X, StartPoint.Y, StartPoint.Z];
                ref var end = ref Grid[EndPoint.X, EndPoint.Y, EndPoint.Z];
                ...
                while (OpenQueue.Count > 0)
                {
                    ...
                    ref var current = ref Grid[g.X, g.Y, g.Z];
                    ...
                    for (var z = zMin; z <= zMax; z++)
                    {
                        ...
                        ref var neighbor = ref Grid[x, y, z];
            ...
            private void RetracePath(ref GridNode startNode, ref GridNode endNode)
            {
                ...
                while (currentNode.GridPoint != startNode.GridPoint && i < MaxPoints)
                {
                    ...
                    currentNode = ref Grid[currentNode.ParentPoint.X, currentNode.ParentPoint.Y, currentNode.ParentPoint.Z];
                    i++;
                }
