...
namespace Assets.Scripts.Systems
{
    ...
    public class SpawnPlayerSystem : SystemBase
    {
        ...
        {
            var components = new[] {ComponentType.ReadWrite<PlayerTag>(), ComponentType.ReadWrite<PlayerSession>(), ComponentType.ReadWrite<PlayerInput>()};
        ...
        protected override void OnUpdate()
        {
            ...
            Entities.ForEach((ref SpawnPlayerEvent e) =>
            {
                ...
                commands.SetComponent(playerEntity, new PlayerSession {PlayerId = id});

                if (e.Category == ActorCategory.AIPlayer)
                    commands.AddComponent<AIPlayerTag>(playerEntity);

                commands.SetComponent(playerEntity, new PlayerInput {InputPlayerId = e.InputPlayerId});
    ...
    public class InitializePlayerSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            {
                players.Ref.Add(new PlayerRef {Id = session.PlayerId, Entity = entity});

                commands.RemoveComponent<UnprocessedTag>(entity);

                events.Enqueue(new PlayerCreatedEvent {Id = session.PlayerId});
            }).WithoutBurst().Run();
