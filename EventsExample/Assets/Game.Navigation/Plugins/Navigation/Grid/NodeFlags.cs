...
namespace Providers.Grid
{
    ...
    public class NodeFlagsDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // var a = (uint) EditorGUI.MaskField(position, label, property.intValue, property.enumNames);
            // if (EditorGUI.EndChangeCheck())
            //     property.intValue = (int) a;

            var t1 = property.name;
            var t2 = property.type;
            var t3 = property.enumNames;
            var t4 = property.intValue;
            var t5 = property.propertyPath;
            var t6 = property.propertyType;
           // var t7 = property.managedReferenceFieldTypename;
            //var t8 = property.managedReferenceFullTypename;
            
            var isChildStruct = fieldInfo.FieldType.IsValueType;
            if (isChildStruct && property.hasChildren)
            {
                foreach (var child in property.GetChildren())
                {
                    if(child.serializedObject.GetType() == typeof(NodeFlags))
                        Debug.Log("NodeFlags field found");
                }
            }
            
            label = EditorGUI.BeginProperty(position, label, property);
            
            //var oldValue = (Enum)fieldInfo.GetValue(property.serializedObject.);
            var oldValue = (NodeFlags)property.longValue;
