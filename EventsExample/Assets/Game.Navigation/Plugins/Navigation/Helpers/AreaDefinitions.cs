...
namespace Providers.Grid
{
    ...
    public class AreaDefinitions : INativeArrayProducer<NativeAreaDefinition>
    {
        ...
        {
            var nativeItems = Items.Values.Select(v => new NativeAreaDefinition {Index = v.Index, Weight = v.Weight, FlagValue = v.FlagValue}).ToArray();
    ...
    public class AreaDefinitions<T> : AreaDefinitions, IEnumerable<AreaDefinition<T>> where T : struct, Enum
    {
        ...
        {
            Items = ((T[]) Enum.GetValues(typeof(T))).ToDictionary(k => (Enum) k, v => (AreaDefinition) AreaDefinition.FromFlag(v, 0f));
        }
        ...
        public IEnumerator<AreaDefinition<T>> GetEnumerator()
        {
            return Items.Values.OfType<AreaDefinition<T>>().GetEnumerator();
        }
