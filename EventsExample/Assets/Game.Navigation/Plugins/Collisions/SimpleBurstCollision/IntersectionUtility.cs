...
namespace Vella.SimpleBurstCollision
{
    public static class IntersectionUtility
    {
        ...
        public static bool ConvexMeshIntersects(Mesh objectA, float4x4 toWorldMatrixA, Mesh objectB, float4x4 toWorldMatrixB)
        {
            ...
            for (var i = 0; i < objectA.triangles.Length; i = i + 3)
            {
                var a1 = aWorldSpaceVerts[objectA.triangles[i]];
                var a2 = aWorldSpaceVerts[objectA.triangles[i + 1]];
                var a3 = aWorldSpaceVerts[objectA.triangles[i + 2]];
            ...
            for (var i = 0; i < objectB.triangles.Length; i = i + 3)
            {
                var a1 = bWorldSpaceVerts[objectB.triangles[i]];
                var a2 = bWorldSpaceVerts[objectB.triangles[i + 1]];
                var a3 = bWorldSpaceVerts[objectB.triangles[i + 2]];
        ...
        private static List<Face> GetFaces(Mesh objectA, float4x4 toWorldMatrixA)
        {
            ...
            for (var i = 0; i < objectA.triangles.Length;)
            {
                var idx1 = i++;
                var idx2 = i++;
                var idx3 = i++;

                var p1 = verts[indices[idx1]];
                var p2 = verts[indices[idx2]];
                var p3 = verts[indices[idx3]];

                var center = (p1 + p2 + p3) / 3;
                var norm = Vector3.Cross(p3 - p2, p1 - p2).normalized;

                result.Add(new Face {Center = math.transform(toWorldMatrixA, center), Normal = math.rotate(toWorldMatrixA, norm.normalized)});
            }
