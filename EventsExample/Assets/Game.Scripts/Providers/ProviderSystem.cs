...
namespace Assets.Scripts.Providers
{
    ...
    public readonly unsafe struct Unmanaged<T> where T : unmanaged
    {
        ...
        public Unmanaged(Allocator allocator)
        {
            Ptr = (T*) UnsafeUtility.Malloc(sizeof(T), UnsafeUtility.AlignOf<T>(), allocator);
        }
        ...
        public static implicit operator Native<T>(Unmanaged<T> o)
        {
            return UnsafeUtilityEx.AsRef<Native<T>>(o.Ptr);
        }
    }
    ...
    public readonly unsafe struct Native<T> where T : struct
    {
        ...
        {
            Ptr = (byte*) UnsafeUtility.Malloc(UnsafeUtility.SizeOf<T>(), UnsafeUtility.AlignOf<T>(), allocator);
        }
        ...
        public ref T1 As<T1>() where T1 : struct
        {
            return ref UnsafeUtilityEx.AsRef<T1>(Ptr);
        }
    }

    public class Provider<T> : IProvider<T>, IProvider where T : struct, INativeProvider
    {
